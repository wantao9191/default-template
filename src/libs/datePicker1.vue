                ...
                <div class="picker-content" @mousedown.prevent>
                    <div class="picker-item" v-for="(w, i) in weeks" :key="i">
                        ...
                        <div class="picker-day" :class="{ 'out-day': d.month != month }" v-for="(d, n) in w.days"
                            ...
                            <span :class="{
                                active: propValueComputed === d.label,
            ...
            <div class="month-wrap" v-if="propType === 'year'" @mousedown.prevent>
                ...
                <div class="month-content">
                    <div v-for="m in years.arrs" class="month-item" :key="m" @click="onYearClick(m)">
                        <span :class="{
                            active: today.selectYear == m || value == m,
...
const yearScope = ref(props.value)
const propType = ref(props.type)
const date = propValueComputed.value ? ref(new Date(props.value)) : ref(new Date())
...
const onMonthClick = (m, close = true) => {
    month.value = m
    if (props.type === 'month') {
        emit('update:value', `${year.value}-${m}`)
        emit('change', `${year.value}-${m}`)
        inputRef.value.blur()
        return
    }
    if (close) propType.value = 'date'
    for (let k in weeks.value) {
        let item = weeks.value[k].days.find(d => {
            return d.month === m && d.day === 1
        })
        if (item) {
            onDayClick({ ...item }, false)
            return
        }
    }
}
const onYearClick = m => {
    ...
    today.selectYear = m
    if (props.type === 'year') {
        emit('update:value', String(m))
        emit('change', String(m))
        inputRef.value.blur()
    } else {
...
const prev = (type) => {
    if (type === 'month') {
        ...
        if (propType.value === 'year') {
            yearScope.value = yearScope.value - yearScope.value % 1000 % 10 - 1
        } else {
            year.value--
        }
...
const next = type => {
    if (type === 'month') {
        ...
        if (propType.value === 'year') {
            yearScope.value = yearScope.value - yearScope.value % 1000 % 10 + 10
        } else {
            year.value++
        }
...
const weeks = computed(() => new datePicker({ year: year.value, month: month.value, format: props.format, type: props.type }).calendar)
const years = computed(() => {
    let yaerValue = year.value
    let arrs = []
